...
func Test(t *testing.T) {
	...
	sc := NewSessionCache[string](
		NewDataManager[string](mfs, testPersist{}), 10*time.Minute)
	...
	_, err = sc.CreateSessionId("test", "test2")
	assert.Error(t, err)
	assert.True(t, strings.Contains(err.Error(), "wrong password"))

	// access existing session
	id2, err := sc.CreateSessionId("test", "test")
	assert.NoError(t, err)
	...
	id3, err := sc.CreateSessionId("test", "test")
	assert.NoError(t, err)
	// new session id
